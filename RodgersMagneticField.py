...
def F(vx,vy,vz,t,m,c,k):
    dt = 0.01
    t = t + dt
    Q = 1.0 #Charge of the particle
    Bx = 1.0 #x-component of the magnetic field
    By = 1.0 #y-component of the magnetic field
    Bz = 1.0 #z-component of the magnetic field
    v = sqrt(vx**2 + vy**2 + vz**2)
    #acceleration terms
         #Q*(v cross B)### 
    Fx = Q*(vy*Bz - vz*By) - c*vx*v
    Fy = Q*(vz*Bx - vx*Bz) - c*vy*v - m*9.8
    Fz = Q*(vx*By - vy*Bx) - c*vz*v
...
Volume_init = (4.0/3.0)*pi*R_init**3
Density_init = m_ball_init*Volume_init
...
for i in values3:
    ...
    while (state == True):
        ...
        m_ball = m_ball + (pi*r**2)*(vel*dt)*density_mist

        R = (3.0*m_ball/(density_ball*4.0*pi))**(1.0/3.0)

        ball.radius = R

        c = density_mist*1.55E-4*2*R

        density_ball = (3.0/4.0)*m_ball/(pi*R**3)

        #Here are the calls for the different components of the original force
        F0x = F(vx,vy,vz,t,m_ball,c,1)
        F0y = F(vx,vy,vz,t,m_ball,c,2)
        F0z = F(vx,vy,vz,t,m_ball,c,3)
        ...
        Fgx = F(vxg,vyg,vzg,t,m_ball,c,1)
        Fgy = F(vxg,vyg,vzg,t,m_ball,c,2)
        Fgz = F(vxg,vyg,vzg,t,m_ball,c,3)        
