...
for i in values1:
    rand_var = 10*random()
    show_num = math.floor(rand_var) + 5 #Ensures that the velocities are never less than 5
    vals1[i] = show_num

#Generates the numbers that will be used in the y velocity
for i in values2:
    rand_var = 10*random()
    show_num = math.floor(rand_var) + 10 #Ensures that the velocities are never less than 10
    vals2[i] = show_num
